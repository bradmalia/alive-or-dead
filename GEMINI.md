...
- **Image Integration:** Gemini embeds portrait image URLs directly in the returned HTML fragments; the backend validates but does not fetch or rewrite them.
...
6. **Embedded Images:** Guessing UIs must include a portrait image via an embedded `http`/`https` URL, and image markup should live inside styled design containers.
